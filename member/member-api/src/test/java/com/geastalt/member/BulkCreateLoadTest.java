 ...
 *
 * Usage: java BulkCreateLoadTest [totalMembers] [batchSize] [host] [port]
 *
 * Defaults:
 *   totalMembers = 10000
 *   batchSize = 100
...
public class BulkCreateLoadTest {
    ...
    public static void main(String[] args) throws Exception {
        int totalMembers = args.length > 0 ? Integer.parseInt(args[0]) : 10000;
        ...
        System.out.printf("Total Members: %,d%n", totalMembers);
        System.out.printf("Batch Size: %d%n", batchSize);
        System.out.printf("Number of Batches: %d%n", (totalMembers + batchSize - 1) / batchSize);
        System.out.println("=".repeat(70));
        ...
        try {
            ...
            runBulkCreateTest(stub, totalMembers, batchSize);
    ...
    private static void runBulkCreateTest(MemberServiceGrpc.MemberServiceBlockingStub stub,
                                           int totalMembers, int batchSize) {
        AtomicInteger totalSuccess = new AtomicInteger(0);
        AtomicInteger totalFailure = new AtomicInteger(0);
        AtomicLong totalTimeMs = new AtomicLong(0);

        int batches = (totalMembers + batchSize - 1) / batchSize;
        ...
        for (int batch = 0; batch < batches; batch++) {
            ...
            // Generate fake members for this batch
            List<MemberInput> members = generateFakeMembers(currentBatchSize);

            // Enable pending actions - Kafka should be available
            BulkCreateMembersRequest request = BulkCreateMembersRequest.newBuilder()
            ...
            try {
                BulkCreateMembersResponse response = stub.bulkCreateMembers(request);
                ...
                System.out.printf("Batch %d/%d: %d members in %dms (%.1f/sec) - Success: %d, Failed: %d%n",
        ...
        System.out.println("=".repeat(70));
        System.out.printf("Total Members Requested: %,d%n", totalMembers);
        ...
        System.out.printf("Overall Rate: %.1f members/second%n",
                totalMembers / (overallElapsed / 1000.0));
        System.out.printf("Average Batch Time: %.0f ms%n",
                totalTimeMs.get() / (double) batches);
        System.out.println("=".repeat(70));
    }

    private static List<MemberInput> generateFakeMembers(int count) {
        List<MemberInput> members = new ArrayList<>(count);

        for (int i = 0; i < count; i++) {
            MemberInput.Builder memberBuilder = MemberInput.newBuilder()
            ...
            if (faker.random().nextDouble() < 0.7) {
                ...
                for (int p = 0; p < phoneCount; p++) {
                    memberBuilder.addPhones(PhoneInput.newBuilder()
            ...
            if (faker.random().nextDouble() < 0.8) {
                ...
                for (int e = 0; e < emailCount; e++) {
                    memberBuilder.addEmails(EmailInput.newBuilder()
            ...
            if (faker.random().nextDouble() < 0.6) {
                ...
                for (int a = 0; a < addressCount; a++) {
                    ...
                    memberBuilder.addAddresses(addressBuilder.build());
                }
            }
